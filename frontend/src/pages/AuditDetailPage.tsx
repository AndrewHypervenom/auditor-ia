...
export default function AuditDetailPage() {
  ...
  // Costos: solo superadmin y líder (supervisor). El auditor NO ve costos.
  ...
  return (
    <div className="min-h-screen">
      ...
      <motion.main initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, ease: [0.16, 1, 0.3, 1], delay: 0.06 }} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5">
        ...
        {/* Desglose de costos de APIs (Claude vs AssemblyAI) — solo superadmin y líder */}
        {(isAdmin || isSupervisor) && auditDetail.apiCosts && (
          <motion.div
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
        ...
        <ResultsView
          ...
          auditId={auditId}
          caseId={auditDetail.audit?.gpf_data?.attentionFields?.['Caso'] ?? auditDetail.audit?.executive_id}
