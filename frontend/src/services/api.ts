...
const getApiBaseUrl = (): string => {
  ...
  console.error('⚠️ VITE_API_URL not configured! Please set it in Vercel environment variables');
  return 'http://localhost:3000';
};
...
const normalizeAudit = (audit: any): Audit => {
  if (!audit) {
    console.warn('⚠️ Attempted to normalize null/undefined audit');
    return audit;
  }
...
export const auditService = {
  ...
  async getUserAudits(limit?: number, offset?: number): Promise<{ audits: Audit[]; total: number }> {
    try {
      ...
      if (!data || typeof data !== 'object') {
        console.warn('⚠️ Invalid response format from /audits:', data);
  ...
  async getMyAudits(): Promise<{ audits: Audit[] }> {
    try {
      ...
      if (!data || typeof data !== 'object') {
        console.warn('⚠️ Invalid response format from /audits/my-audits:', data);
        return { audits: [] };
      }
