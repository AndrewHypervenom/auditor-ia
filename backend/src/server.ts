...
// ============================================
// SEED DEFAULTS — Popula BD con datos estáticos
// ============================================

app.post('/api/admin/seed-defaults', authenticateUser, requireAdmin, async (req: Request, res: Response) => {
  try {
    const results: Record<string, string> = {};

    // --- Seed scripts ---
    const existingScripts = await databaseService.getAllScripts();
    if (existingScripts.length === 0) {
      const { SCRIPT_FRAUDE_STEPS, SCRIPT_MONITOREO_STEPS, SCRIPT_TH_CONFIRMA_STEPS } =
        await import('./config/scripts-content.js');

      const buildInserts = (callType: string, steps: Record<string, string[]>) =>
        Object.entries(steps).map(([key, lines], idx) => ({
          call_type: callType,
          step_key: key,
          step_label: key.replace(/_/g, ' ').replace(/\b\w/g, (c: string) => c.toUpperCase()),
          step_order: idx + 1,
          lines
        }));

      const allScripts = [
        ...buildInserts('FRAUDE', SCRIPT_FRAUDE_STEPS as Record<string, string[]>),
        ...buildInserts('MONITOREO', SCRIPT_MONITOREO_STEPS as Record<string, string[]>),
        ...buildInserts('TH CONFIRMA', SCRIPT_TH_CONFIRMA_STEPS as Record<string, string[]>),
      ];

      for (const s of allScripts) {
        await databaseService.createScript(s);
      }
      results.scripts = `Insertados ${allScripts.length} pasos de script`;
    } else {
      results.scripts = `Omitido: ya existen ${existingScripts.length} scripts en BD`;
    }

    // --- Seed criteria ---
    const existingBlocks = await databaseService.getAllCriteriaBlocks();
    if (existingBlocks.length === 0) {
      const { getCriteriaForCallType } = await import('./config/evaluation-criteria.js');

      const seedForCallType = async (callType: string) => {
        const blocks = getCriteriaForCallType(callType);
        for (let i = 0; i < blocks.length; i++) {
          const block = blocks[i];
          const createdBlock = await databaseService.createBlock({
            call_type: callType,
            block_name: block.blockName,
            block_order: i + 1,
          });
          for (let j = 0; j < block.topics.length; j++) {
            const t = block.topics[j];
            await databaseService.createCriteria({
              block_id: createdBlock.id,
              topic: t.topic,
              criticality: t.criticality,
              points: t.points === 'n/a' ? null : (t.points as number),
              applies: t.applies,
              what_to_look_for: (t as any).whatToLookFor || null,
              criteria_order: j + 1,
            });
          }
        }
      ...
      await seedForCallType('FRAUDE');
      await seedForCallType('MONITOREO');
      await seedForCallType('TH CONFIRMA');
      results.criteria = 'Criterios sembrados para FRAUDE, MONITOREO y TH CONFIRMA';
    } else {
      results.criteria = `Omitido: ya existen ${existingBlocks.length} bloques en BD`;
    }

    databaseService.invalidatePromptsCache();
    res.json({ success: true, results });
  } catch (error: any) {
    logger.error('Error seeding defaults:', error);
    res.status(500).json({ error: 'Error al cargar datos predeterminados', detail: error.message });
  }
});
